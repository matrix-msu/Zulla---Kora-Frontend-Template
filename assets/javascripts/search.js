...
var createCards = function(terms) {
    if (terms.length === 1) {
        ...
        $("<div class='searched-term'>" + terms + "<img class='close' src='" + BASE_TEMPLATE_IMAGE_URL + "x.svg' alt='close button'></div>").appendTo("div.search-field");
    } else if (terms.length > 1) {
        for (var i = 0; i < terms.length; i++) {
            // append the term to the div, append div to parent element
            $("<div class='searched-term'>" + terms[i] + "<img class='close' src='" + BASE_TEMPLATE_IMAGE_URL + "x.svg' alt='close button'></div>").appendTo("div.search-field");
        }
    }
    setPositioning ()
}
